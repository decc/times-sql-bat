...
echo ) TO '%~dp0landfillemiss.csv' delimiter ',' CSV HEADER; >> AgBatchUpload.sql
...
echo select tablename, attribute, period,pv, >> AgBatchUpload.sql
echo case >> AgBatchUpload.sql
echo when process in ('ALUFOR01','ALUFOR02','ALUFOR03','ALUFOR04A','ALUFOR04B') then 'affor' >> AgBatchUpload.sql
...
echo order by tablename, attribute, proc_set >> AgBatchUpload.sql
echo ) TO '%~dp0lulucfout.csv' delimiter ',' CSV; >> AgBatchUpload.sql
...
echo sum(case when period='2060' then pv else 0 end)::numeric "2060" >> AgBatchUpload.sql
echo from vedastore >> AgBatchUpload.sql
echo where attribute='VAR_FOut' and commodity='ALAND' and process in('ALUFOR01','ALUFOR02','ALUFOR03','ALUFOR04A') >> AgBatchUpload.sql
...
"C:\Program Files\PostgreSQL\%postgresver%\bin\psql.exe" -h localhost -p 5432 -U postgres -d postgres -f %~dp0AgBatchUpload.sql
rem following concatenates individual results to the landfillemiss.csv
type lulucfout.csv >> landfillemiss.csv
type afforestout.csv >> landfillemiss.csv
...
rename landfillemiss.csv AgResultsOut.csv
